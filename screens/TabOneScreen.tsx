...
const MusicCard: FC<MusicCardProps> = (props) => {
  return (
    <Pressable onPress={props.addSongRequest}>
      <View style={tailwind('mx-4 my-4 h-40 rounded-3xl overflow-hidden')}>
        <ImageBackground
          source={{ uri: props.thumbnailImage }}
          blurRadius={40}
          style={tailwind('h-full w-full flex flex-row')}
        >
          <View
            style={tailwind(
              'h-full w-32 justify-center items-center bg-transparent opacity-80'
            )}
          >
            <Image
              source={{ uri: props.thumbnailImage }}
              style={tailwind('h-56 w-32')}
              resizeMode='cover'
            />
          </View>

          <View style={tailwind('flex-1 p-4 bg-transparent')}>
            <Text style={tailwind('font-semibold text-white text-lg')}>
              {props.title.slice(0, 40) +
                (props.title.length > 40 ? '...' : '')}
            </Text>
            <Text style={tailwind('text-gray-300')}>{props.channel}</Text>
          </View>
        </ImageBackground>
      </View>
    </Pressable>
  ...
  return (
    <View style={tailwind('h-full')}>
      <View style={tailwind('flex flex-1')}>
        <View
          style={tailwind(
            'h-24 border-b border-gray-200 flex justify-end items-end pb-2 px-4'
          )}
        >
          <View style={tailwind('flex flex-row')}>
            ...
            <View style={tailwind('w-6 h-6 rounded-full bg-gray-300 ml-2')} />
          </View>
        </View>
        ...
        <View
          style={tailwind(
            'flex flex-row items-center h-12 mx-4 my-3 rounded-xl bg-gray-50'
          )}
        ...
        >
          <Ionicons
            ...
            size={20}
            color='#94a3b8'
            style={tailwind('ml-3 absolute z-20')}
          />
          <TextInput
            ref={keywordInputRef}
            style={tailwind('h-full w-full pl-9 pr-3')}
            placeholder='Search'
